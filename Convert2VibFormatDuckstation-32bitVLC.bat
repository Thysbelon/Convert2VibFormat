...
FOR %%i IN (*.wav *.mp3 *.m4a *.ogg *.flac *.aif *.aiff *.aac *.dts *.wma *.wma1 *.wma2 *.alac) DO (
	...
	"C:\Program Files (x86)\VideoLAN\VLC\vlc.exe" -I dummy --dummy-quiet "%%~nxi" --sout=#transcode{vcodec=none,acodec=s16l,ab=128,channels=2,samplerate=44100,scodec=none}:std{access=file{no-overwrite},mux=wav,dst="temp.bin"} vlc://quit
