...
for i in *.{wav,mp3,m4a,ogg,flac,aif,aiff,aac,dts,wma,wma1,wma2,alac}; do 
	echo "$i"
	cvlc "$i" --sout='#transcode{vcodec=none,acodec=s16l,ab=128,channels=2,samplerate=44100,scodec=none}:std{access=file{no-overwrite},mux=wav,dst=temp.bin}' vlc://quit
	...
	[[ "$chdpref" = "y" ]] && chdman createcd -i temp.cue -o "${i%.*}.chd" && if [ $? -gt 0 ]; then read -p "chdman had an error. if chdman is not installed, please install mame-tools." && shopt -u nullglob && exit 1; fi
